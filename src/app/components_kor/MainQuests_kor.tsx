        ...
        <Carousel>
            {
                mainQuestArticles.map((article, index) => {
                    return (
                        <Card key={index} $currentPage={currentPage} onClick={() => {navigate(`article_kor?id=${article.id}`)}}>
                            <div className='article-space'>
                                <div>
                                <p className='title'>{article.title}</p>
                                {article.summary}
                                </div>
                            <Image height={800} width={1000} className='primary-image' src={article.primaryImage} 
                            alt='primary_image'/>
                            </div>
                            <div className='box'>
